...
function awp_open_div() {
	echo '<div class="featured-community awp-community">'; }

// close the div and class for the custom post type archive
add_action( 'genesis_after_loop', 'awp_close_div' );
function awp_close_div() {
	echo '</div> <!-- close featured community div -->' ; }
...
function genawpcomm_awp_community_grid() {
	...
	$image = genesis_get_image( 'format=url&size=awp-feature-community&fallback=' );
